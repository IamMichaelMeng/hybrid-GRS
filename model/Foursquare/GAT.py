...
def main():
    ...
    graph = create_graph('datasets/Foursquare/Foursquare_data_size.txt', 'datasets/Foursquare/Foursquare_social_relations.txt')
    # calculate graph node influence
    node_influence = get_node_influence(graph) # user 818 is the boss

    # create user node embedding
    user_info = np.loadtxt('datasets/Foursquare/Foursquare_user_features.txt', dtype=np.float32)
    ...
    np.savetxt('datasets/group/Foursquare_GAT_trained_user_features.txt', best_embedding)

    # aggregate user emb to get group emb
    group_emb = aggregate_user_embedding(node_influence, 'datasets/group/Foursquare_GAT_trained_user_features.txt')
    np.savetxt('datasets/group/Foursquare_group_emb.txt',  group_emb, fmt='%f')
