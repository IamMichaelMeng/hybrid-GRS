...
def train(dataset, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            pdb.set_trace()
            opt.zero_grad()
            pred = model(batch)  # 获取每一行样本的训练标签
            label = batch.y  # 获取每一行样本的真实标签
...
class GNNStack(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.post_mp(x) # 将x从[2708, 64]降至[2708, 7],每一列对应7个classes的概率大小，取列最大值就是对应的classes

        if self.emb == True:
            return x

        return F.log_softmax(x, dim=1)  # 对每一行进行log_softmax运算，并使得每一行的所有元素和为1
...
def main():
    for args in [
        {'model_type': 'GAT', 'dataset': 'Foursquare', 'num_layers': 2, 'heads': 1,'num_classes': 'batch_size': 32, 'hidden_dim': 32, 'dropout': 0.5, 'epochs': 500, 'opt': 'adam', 'opt_scheduler': 'none', 'opt_restart': 0, 'weight_decay': 5e-3, 'lr': 0.01}
    ]:
