...
def calculate_performance(prediction_data, actual, similarity_cate):
    ...
    file = open('../metric/metric/Gowalla_result_'+timestamp+'.txt', mode = 'a')
...
def read_prediction_data():
    data_list = []
    data = open('../results/Gowalla_result_2022-05-15-19-07-25.txt', 'r').readlines()
...
if __name__=='__main__':
    ...
    data_path = [
    '../datasets/group/group_poi/Gowalla_enclidean_single_positive_group_poi.txt',
    '../datasets/group/group_poi/Gowalla_cosine_single_positive_group_poi.txt',
    '../datasets/group/group_poi/Gowalla_person_single_positive_group_poi.txt']
    # 计算准确度、召回度以及平均损失
    for i in range(3):
