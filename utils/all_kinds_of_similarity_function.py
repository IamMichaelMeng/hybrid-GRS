...
def create_nagative_interection(positive_interection, poi_num, dataset_name):
    file = open('../datasets/group_poi/'+ dataset_name +'_negative_group_poi.txt', mode='a')
    group_id = -1
...
def get_common_user_poi_interection(user_id):
    groud_truth = set() 
    truth_data = open('../datasets/Foursquare/Foursquare_check_ins.txt', 'r').readlines()
...
def create_single_positive_group_poi_interaction(user_ids, data_name):
    ...
    file = open('../datasets/group_poi/'+ data_name +'_single_positive_group_poi.txt', mode='a')
...
def create_positive_group_poi_interaction(euclidean_dis_set, cosine_similarity_set, person_coefficient_set, data_name):
    ...
    for user_id in tqdm(common_user_id):
        interection_data = get_common_user_poi_interection(user_id)
        common_user_poi_interection_set.update(interection_data)
    file = open('../datasets/group_poi/'+ data_name +'_positive_group_poi.txt', mode='a')
    group_id = -1 # set group_id = -1
    for elem in common_user_poi_interection_set:
        file.writelines([str(group_id)+' ', str(elem)+' ', str(1), '\n'])
    file.close()
...
def euclidean_distance():
    ...
    # 将结果保存起来
    np.savetxt('../datasets/group_poi/euclidean_dis_set.txt', euclidean_dis_set, fmt='%d')
...
def cosine_similarity():
    ...
    # 将结果保存起来
    np.savetxt('../datasets/group_poi/cosine_dis_set.txt', cosine_similarity_set, fmt='%d')
...
def person_coefficient():
    ...
    # 将结果保存起来
    np.savetxt('../datasets/group_poi/person_dis_set.txt', person_coefficient_set, fmt='%d')
...
if __name__ =='__main__':
    ...
    user_features_file_path.append('../datasets/Foursquare/Foursquare_user_features.txt')
    ...
    '''
    positive_interection = create_positive_group_poi_interaction(euclidean_dis_set, cosine_similarity_set, person_coefficient_set, dataset_name[0])    
    create_negative_group_poi_interaction(dataset_name[0], positive_interection)
