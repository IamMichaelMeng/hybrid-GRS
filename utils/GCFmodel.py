...
class GCF(Module):
    ...
    def __init__(self,userNum,itemNum,rt,poi_features,context_features,embedSize,layers,useCuda=False):
        ...
        self.uEmbd.weight.data = torch.tensor(poi_features, requires_grad=True)
        self.iEmbd.weight.data = torch.tensor(context_features, requires_grad=True)
        # userNum对应poiNum, poiNum对应contextNum
        #self.uEmbd = nn.Embedding(userNum,embedSize)
        #self.iEmbd = nn.Embedding(itemNum,embedSize)
    ...
    def buildLaplacianMat(self,rt):
        ...
        uiMat = coo_matrix((rt['frequency'], (rt['userId'], rt['itemId'])))

        uiMat_upperPart = coo_matrix((rt['frequency'], (rt['userId'], rt_item)))
    ...
    def forward(self,userIdx,itemIdx):
        ...
        prediction = embd.flatten() # 返回一个一维数组
