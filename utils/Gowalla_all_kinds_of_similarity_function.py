...
def create_nagative_interection(positive_interection, poi_num, dataset_name):
    file = open('../datasets/group/group_poi/Gowalla_'+ dataset_name +'_negative_group_poi.txt', mode='a')
    group_id = -1
...
def create_negative_group_poi_interaction(dataset_name, positive_interection):
    if dataset_name == 'Gowalla':
        data_size = np.loadtxt('../datasets/Gowalla/Gowalla_data_size.txt', dtype=np.int32)
        poi_num = data_size[1]
        create_nagative_interection(positive_interection, poi_num, dataset_name)

def get_common_user_poi_interection(user_id):
    groud_truth = set() 
    truth_data = open('../datasets/Gowalla/Gowalla_checkins.txt', 'r').readlines()
...
def create_single_positive_group_poi_interaction(user_ids, data_name):
    ...
    file = open('../datasets/group/group_poi/Gowalla_'+ data_name +'_single_positive_group_poi.txt', mode='a')
...
def create_positive_group_poi_interaction(euclidean_dis_set, cosine_similarity_set, person_coefficient_set, data_name):
    ...
    for user_id in tqdm(common_user_id):
        interection_data = get_common_user_poi_interection(user_id)
        common_user_poi_interection_set.update(interection_data)
    file = open('../datasets/group/group_poi/Gowalla_'+ data_name +'_positive_group_poi.txt', mode='a')
    group_id = -1 # set group_id = -1
    for elem in common_user_poi_interection_set:
        file.writelines([str(group_id)+' ', str(elem)+' ', str(1), '\n'])
    file.close()
...
def euclidean_distance():
    ...
    # 将结果保存起来
    np.savetxt('../datasets/group/group_poi/Gowalla_euclidean_dis_set.txt', euclidean_dis_set, fmt='%d')
...
def cosine_similarity():
    ...
    # 将结果保存起来
    np.savetxt('../datasets/group/group_poi/Gowalla_cosine_dis_set.txt', cosine_similarity_set, fmt='%d')
...
def person_coefficient():
    ...
    # 将结果保存起来
    np.savetxt('../datasets/group/group_poi/Gowalla_person_dis_set.txt', person_coefficient_set, fmt='%d')
